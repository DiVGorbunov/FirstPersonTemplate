...
public class PlayerWeaponsManager : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (activeWeapon && m_WeaponSwitchState == WeaponSwitchState.Up)
        {
            ...
            bool altHasFired = false;

            if (activeWeapon.isAltFireAvailable)
            {
                altHasFired = activeWeapon.HandleShootInputs(
                m_InputHandler.GetAltFireInputDown(),
                m_InputHandler.GetAltFireInputHeld(),
                m_InputHandler.GetAltFireInputReleased(), true);
                isAiming = false;
            }

            // Handle accumulating recoil
            if (hasFired || altHasFired)
            {
