...
public class PlayerInputHandler : MonoBehaviour
{
    ...
    public bool GetAltFireInputHeld()
    {
        if (CanProcessInput())
        {
            bool isGamepad = Input.GetAxis(GameConstants.k_ButtonNameGamepadAltFire) != 0f;
            if (isGamepad)
            {
                return Input.GetAxis(GameConstants.k_ButtonNameGamepadAltFire) >= triggerAxisThreshold;
            }
            else
            {
